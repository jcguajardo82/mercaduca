...
function llenaCombo(idCmb, respon, datos) {
    debugger;
    //Se agrega que limpie siempre el combo para que no se queden datos guardados (mejora) - MP - 10/04/2019
    if (document.getElementById(idCmb).length > 0) {
        setValue(idCmb, "");
    }

    var cmb = respon[datos].split("*");
    var select = document.getElementById(idCmb); //Seleccionamos el select

    for (var i = 0; i < cmb.length; i++) {
        if (cmb[i].split(":")[1] != "" && cmb[i].split(":")[1] != undefined) {
            var option = document.createElement("option"); //Creamos la opcion
            option.innerHTML = cmb[i].split(":")[1]; //Metemos el texto en la opción
            option.value = cmb[i].split(":")[0];
                        ...
                        switch (inputField) {
                            ...
                            case "inLot":
                                ...
                                if (xmlHttp.responseText.indexOf("Continuar") > 0) {
                                    ...
                                    setValueName("lblLote", document.getElementById(inputField).value);
